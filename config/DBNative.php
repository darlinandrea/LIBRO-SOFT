...
class DBNative
{
	...
	private $db;
	public $DSN;
	public function DBNative($DSN = false, $host = false, $user = false, $passwd = false,
		$db = false)
	{
		$this->db = $db;
		$this->DSN = $DSN;
		...
		if (!empty($DSN))
		{
			...
			if (!empty($host) && !empty($user) && !empty($passwd) && !empty($db))
			{
				...
				die("Please your must to pass either the DSN String or the Connection Parameters");
		$this->db = $aTmp["db"];
	}
	/**
	...
	public function query($SQL,$numericArray=0)
	{
		...
		if ($result === FALSE)
		{
			...
			echo "<pre>";print_r($this).debug_print_backtrace(); echo "</pre>";
	...
	{
		$values = implode(",", array_map(array($this, "quote"), array_values($fields)));
	...
	public function autoUpdate($fields, $table, $whereString, $autoQuote = true)
	{
		...
		foreach ($fields as $name => $value)
			$sqlA[] = "`" . $name . "` = " . ($autoQuote ? $this->quote($value) : $value);
